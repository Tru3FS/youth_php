...
$('.tabmenu2 li').each(function(){
       ...
       $( '#tag_area' ).css("display","block");
              ...
              $.ajax({
	               ...
	               dataType : 'html',
	               data : {'item2':'<?php echo $s_code;?>', 'target':'', 'area':$(this).find('a').data("id")},
	               async:false,
				   cache : false,
				...
				$('#subStepList').html(data);
                 ...
                 $.ajax({
	               ...
	               dataType : 'html',
	               data : {'item2':'<?php echo $s_code;?>', 'target':'', 'learnstep':'<?php echo $g_code;?>', 'area':'<?php echo $center_id;?>'},
	               async:false,
                   cache : false,
	               contentType:"application/x-www-form-urlencoded; charset=UTF-8",
	               success : function(data){
			...
			<?php if($center_id !='' && $s_code==""  && $g_code=="" && $b_code==""  ){ ?>
 ...
 $( '#tag_area' ).css("display","none");
        ...
        <?php }else if($center_id !='' && $s_code!=""  && $g_code=="" && $b_code==""  ){ ?>
 ...
 $( '#tag_area' ).css("display","block");
         ...
         $('.btn_L_col1').attr('disabled', false);
	               ...
	               $.ajax({
						...
						url :  "./s_center/Lecture_Search_List.ajax.php",
						dataType : 'html',
						data : {'item1':$(this).find('a').data("id"),'item2':'<?php echo $s_code;?>','item3':'<?php echo $g_code;?>','target':'','page':'<?php echo $page;?>'},
						async:false,
                        cache : false,
						contentType:"application/x-www-form-urlencoded; charset=UTF-8",
						success : function(data){
...
$('#sSubStepList li.js_select .btn').each(function(){

	console.log($("#sSubStepList  li .btn").val());

if ($("#sSubStepList  li .btn").val()=='<?php echo $b_code;?>')
{
   
   $("#sSubStepList li .btn").addClass("on");
