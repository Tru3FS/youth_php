...
if(get_cookie('ck_visit_ip') != $_SERVER['REMOTE_ADDR'])
{
    ...
    $sql = " insert shop_visit ( vi_id, mb_id, vi_ip, vi_date, vi_time, vi_referer, vi_agent ) values ( '{$vi_id}', '{$pt_id}', '{$remote_addr}', '".NC_TIME_YMD."', '".NC_TIME_HIS."', '{$referer}', '{$user_agent}' ) ";
    ...
    if($result) {
        ...
        if($tmp_cnt) {
			...
			$sql = " insert shop_visit_sum (vs_count, vs_date, mb_id) values (1, '".NC_TIME_YMD."', '{$pt_id}') ";
			sql_query($sql);
		}
        ...
        $sql = " select vs_count as cnt 
				   from shop_visit_sum
				   where vs_date = DATE_SUB('".NC_TIME_YMD."', INTERVAL 1 DAY)
				     and mb_id = '{$pt_id}' ";
        $row = sql_fetch($sql);
