...
// 실행일 비교
if(isset($default['de_optimize_date']) && $default['de_optimize_date'] >= NC_TIME_YMD)
    return;

// 설정일이 지난 장바구니 상품 삭제
if($default['de_cart_keep_term'] > 0) {
	$tmp_before_date = date("Y-m-d", NC_SERVER_TIME - ($default['de_cart_keep_term'] * 86400));
...
if($default['de_wish_keep_term'] > 0) {
	$tmp_before_date = date("Y-m-d", NC_SERVER_TIME - ($default['de_wish_keep_term'] * 86400));
...
if($default['de_final_keep_term'] > 0) {
	$tmp_before_date = date("Y-m-d", NC_SERVER_TIME - ($default['de_final_keep_term'] * 86400));
	$sql = " update shop_order
				set user_ok = '1'
				  , user_date = '".NC_TIME_YMDHIS."'
...
if($default['de_misu_keep_term'] > 0) {
	$tmp_before_date = date("Y-m-d", NC_SERVER_TIME - ($default['de_misu_keep_term'] * 86400));
	$sql = " select *
			   from shop_order
	...
	while($row=sql_fetch_array($res)) {
		...
		// 메모남김
		$sql = " update shop_order
					set shop_memo = CONCAT(shop_memo,\"\\n미입금 자동 주문취소 - ".NC_TIME_YMDHIS." (취소이유 : {$default['de_misu_keep_term']}일경과)\")
...
if(isset($default['de_optimize_date'])) {
    sql_query(" update shop_default set de_optimize_date = '".NC_TIME_YMD."' ");
}
