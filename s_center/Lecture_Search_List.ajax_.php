...
.rank_list2 td{
    height: 70px;
    padding-right: 0px;
    border-top: 1px solid #f3f3f3;    float: left;
}
.rank_list2 td.three{
    height: 210px;
    padding-right: 0px;
...
<div class="board_head ">
	<div class="count_area">
		<p>검색결과 <em id="" class="tot"><?php echo $totalcount;?></em>개</p>
	</div>
	<div class="order_area type3">
		<div class="btn_wrap pad_b0">
...
$t=0;
for($i=1; $row=sql_fetch_array($resultc); $i++) { 
...
$sql="SELECT count(a.Sales_Item_Name) as cnt, a.Sales_Item_Name
   FROM TB_SaleItem       a INNER JOIN
         TB_SaleItem_Price b ON a.Center_ID = b.Center_ID AND a.Sales_Code = b.Sales_Code INNER JOIN
		 TB_SystemSetting  e ON a.Center_ID = e.Center_ID
         WHERE a.Center_ID          = '01'
         AND a.Event_Code        LIKE CONCAT('$event_code', '%')
		 AND a.Class_Code      LIKE CONCAT('$class_code', '%')
		 AND a.Place_Code      LIKE CONCAT('$place_code', '%')
		 AND a.Sales_Place_Code LIKE CONCAT('$sales_place_code', '%') 
	     AND a.Online_Yn          = 'Y'
		 AND b.Online_Yn          = 'Y'
	     AND a.State              = '001'
         AND a.Sales_Division   = '003'
         AND a.First_Start_Day_Yn = 'Y'
	     AND b.Apply_Date          = (SELECT MAX(Apply_Date)  FROM TB_SaleItem	   WHERE Center_ID   = a.Center_ID AND Apply_Date <= F_DATE_TIME('YYYYMMDD') ) GROUP BY a.Sales_Item_Name";
$row2 = sql_fetch($sql);
$total_count2 = $row2['cnt'];
$resultc = sql_query($sql);
...
$sales_name=$row2['Sales_Item_Name'];

$sql2="SELECT count(a.Sales_Item_Name) as cnt
   FROM TB_SaleItem       a INNER JOIN
         TB_SaleItem_Price b ON a.Center_ID = b.Center_ID AND a.Sales_Code = b.Sales_Code INNER JOIN
		 TB_SystemSetting  e ON a.Center_ID = e.Center_ID
         WHERE a.Center_ID          = '01'
         ...
         AND a.Sales_Division   = '003'
         AND a.First_Start_Day_Yn = 'Y'
	     AND b.Apply_Date          = (SELECT MAX(Apply_Date)  FROM TB_SaleItem	   WHERE Center_ID   = a.Center_ID AND Apply_Date <= F_DATE_TIME('YYYYMMDD') ) 
		  AND a.Sales_Item_Name = '$sales_name'";
$row3 = sql_fetch($sql2);
...
$class="two";
}else if($total_count3 == 3){
$class="three";
}else if($total_count3 == 4){
$class="four";
}else if($total_count3 == 5){
$class="five";
}else if($total_count3 == 6){
$class="six";
}else if($total_count3 == 7){
$class="seven";
}else if($total_count3 == 8){
$class="eight";
}else if($total_count3 == 9){
$class="nine";
}else if($total_count3 ==  10){
$class="ten";
}
...
<tr >
        <td class="best <?php echo $class;?> <?php echo $i;?>">
        <div class="wrap"><a href="#!" class="link"><span class="team"><?php echo $center_txt;?></span><span class="player"><?php echo $sales_name;?></span></a></div>
...
?>
    ...
    <th scope="col" colspan="2"><a href="#" class="sort"><span>수강료(개월/원)</span></a></th>
...
?>	
  ...
  <tr>
        ...
        <td><span><?php echo $row2['Month_Qty'];?></span></td>
        
        <td class="align_R"><span class='price'><?php echo number_format($row2['Unit_Price']);?></span></td>
        
        <td><?php echo $s_people;?></td>
        
        <td><a href="#!"><?php echo $s_txt2;?></a><!--<br><?php echo $total_people;?>/<?php echo $cur_people;?>--></td>

        <td>
   
		<a href="#!" onclick="goLink('<?php echo $row2['Sales_Code'];?>','<?php echo urlencode($row2['Event_Name']);?>','<?php echo $row2['Event_Code'];?>','<?php echo $row2['Place_Code'];?>','<?php echo $row2['Class_Code'];?>','<?php echo $center_id;?>','<?php echo $page;?>','<?php echo urlencode($row2['Sales_Item_Name']);?>','<?php echo $row2['Center_ID'];?>')"><?php echo $s_txt;?></a>
...
return results[1] || '';
        ...
        $.post('./s_center/Lecture_Search_List.ajax.php', { action: 'nc_all', 'item1':$.urlParam('item1'),'item2':$.urlParam('item2'),'item3':$.urlParam('item3'),'item4':$.urlParam('item4'),'page':$.urlParam('page')}, function ( data ) {
