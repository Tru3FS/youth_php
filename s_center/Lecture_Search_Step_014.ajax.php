...
$sales_place_code=isset($_POST['area']) ? clean_xss_tags($_POST['area'], 1, 1) : '';
$ip=get_real_client_ip();
$member_id       = get_session('m_id');
$center_id = $_SESSION['center_id'];
$event_code=isset($_POST['item2']) ? clean_xss_tags($_POST['item2'], 1, 1) : '';
...
<form name="quickFrm" id="quickFrm">
	...
	<input type="hidden" name="item21" value="<?php echo $event_code;?>" />
...
if($json_array['Result']['ResultCode'] != 0){
 echo $json_array['Result']['ResultMsg'];
		...
		$event_code=$val['Class_Code'];
		$event_name=$val['Class_Name'];
...
$('.step_box2 .step_items > li:nth-child(3) .title').addClass('');
   ...
   $selector.find( '.second li[data-target="' + $(this).data( 'id' ) + '"]' ).remove();
	...
	var target = $(".chkVal").children().val();
 	$.ajax({
		...
		dataType : 'html',
		data : {'item2':item2, 'target':target, 'learnstep':learnstep, 'area':area, 'search':$('#search').val()},
		async:false,
        cache : false,
		...
		if($("#subSChkVal").val() != undefined || $("#subSChkVal").val() != "" || $("#subSChkVal").val() == ""){
			if($("#subSChkVal2").val() != undefined || $("#subSChkVal2").val() != "" || $("#subSChkVal2").val() == ""){
				if($("#item5").val() != undefined || $("#item5").val() != "" || $("#item5").val() == ""){
					...
					$("#search").val($("#quickFrm [name=search]").val());
