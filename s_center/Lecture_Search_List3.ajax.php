...
$qstr1 = "item1=$center_id&item2=$s_code&item3=$g_code&item4=$b_code&search=$search";
	...
	$sqlb = $sqlb."           AND CASE WHEN a.Event_Code = '009' THEN LEFT(a.Special_Start_Date, 6) ELSE '$today2' END BETWEEN LEFT(Start_Date, 6) AND LEFT(End_Date, 6) ";
	...
	$sqlb = $sqlb."           WHERE Center_ID       = a.Center_ID ";
	$sqlb = $sqlb."             AND YYYYMM          = CASE WHEN a.Event_Code = '009' THEN LEFT(a.Special_Start_Date, 6) ELSE '$today2' END ";
	$sqlb = $sqlb."             AND Sales_Code      = a.Sales_Code ";
	...
	$sqlb = $sqlb."       a.Place_Code, F_NVLS(e.Detail_Name, '') as Place_Name, a.Repeat_Lecture_Yn, ";
	...
	$sqlb = $sqlb."   AND CASE WHEN a.Event_Code = '009' AND F_NVLS(Special_Start_Date, '00000000') <> '00000000' THEN b.Apply_Date    = (SELECT MAX(Apply_Date) FROM TB_SaleItem_Price where Center_ID = a.Center_ID AND Apply_Date <= CONCAT(SUBSTRING(a.Special_Start_Date, 1, 6), '01')) ELSE b.Apply_Date    = (SELECT MAX(Apply_Date) FROM TB_SaleItem_Price where Center_ID = a.Center_ID AND Apply_Date <= CONCAT('$today2', '01')) END  ";
	...
	$sqlc = $sqlc."           AND CASE WHEN a.Event_Code = '009' THEN LEFT(a.Special_Start_Date, 6) ELSE '$today2' END BETWEEN Start_Date AND End_Date ";
	...
	$sqlc = $sqlc."             AND Sales_Division  = '003' ";
	$sqlc = $sqlc."             AND Sales_Code      = a.Sales_Code ";
	$sqlc = $sqlc."             AND CASE WHEN a.Event_Code = '009' THEN LEFT(a.Special_Start_Date, 6) ELSE '$today2' END BETWEEN LEFT(Start_Date, 6) AND LEFT(End_Date, 6) ";
	$sqlc = $sqlc."             AND State           = '001')  ";
	$sqlc = $sqlc."       + (SELECT COUNT(*) ";
	$sqlc = $sqlc."            FROM tb_lecture_waiting ";
	$sqlc = $sqlc."           WHERE Center_ID       = a.Center_ID ";
	$sqlc = $sqlc."             AND YYYYMM          = CASE WHEN a.Event_Code = '009' THEN LEFT(a.Special_Start_Date, 6) ELSE '$today2' END ";
	$sqlc = $sqlc."             AND Sales_Code      = a.Sales_Code ";
	...
	$sqlc = $sqlc."       a.Place_Code, F_NVLS(e.Detail_Name, '') as Place_Name, a.Repeat_Lecture_Yn, ";
	...
	$sqlc = $sqlc."   AND CASE WHEN a.Event_Code = '009' AND F_NVLS(Special_Start_Date, '00000000') <> '00000000' THEN b.Apply_Date    = (SELECT MAX(Apply_Date) FROM TB_SaleItem_Price where Center_ID = a.Center_ID AND Apply_Date <= CONCAT(SUBSTRING(a.Special_Start_Date, 1, 6), '01')) ELSE b.Apply_Date    = (SELECT MAX(Apply_Date) FROM TB_SaleItem_Price where Center_ID = a.Center_ID AND Apply_Date <= CONCAT('$today2', '01')) END  ";
...
<?php
 ...
 <tr class="">
        ...
        <td class="cc <?php echo $sclass;?>"  style="background-color:<?php echo $sbg;?>;">
   <a href="#!" onclick="goLink('<?php echo $row2['Sales_Code'];?>','<?php echo urlencode($row2['Event_Name']);?>','<?php echo $row2['Event_Code'];?>','<?php echo $row2['Place_Code'];?>','<?php echo $row2['Class_Code'];?>','<?php echo $center_id;?>','<?php echo $page;?>','<?php echo urlencode($row2['Sales_Item_Name']);?>','<?php echo $row2['Center_ID'];?>','<?php echo $row3['Month_Qty'];?>','<?php echo $unit_price2;?>')"><?php echo $s_txt;?></a>
