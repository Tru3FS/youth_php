...
include_once(NC_KCPCERT_PATH.'/kcpcert_config.php');
...
$nc['title'] = '휴대폰인증 결과';
include_once(NC_PATH.'/head.sub.php');
...
if( $cert_enc_use == "Y" )
{
    ...
    if( $res_cd == "0000" )
    {
        ...
        if ( $ct_cert->check_valid_hash ( $home_dir , $dn_hash , $veri_str ) != "1" )
        {
            ...
            echo "dn_hash 변조 위험있음 (".NC_KCPCERT_PATH.$bin_exe." 파일에 실행권한이 있는지 확인하세요.)";
            exit;
        ...
        $sql = " select member_id from {nc['member_table']} where member_id <> '{$member['member_id']}' and mb_dupinfo = '{$mb_dupinfo}' ";
        $row = sql_fetch($sql);
        if ($row['member_id']) {
...
<?php
include_once(NC_PATH.'/tail.sub.php');