...
// 인증내역기록
@insert_cert_history($member['memeber_id'], 'kcb', 'ipin');
...
// 중복정보 체크
$sql = " select memeber_id from {$nc['member_table']} where memeber_id <> '{$member['memeber_id']}' and mb_dupinfo = '{$mb_dupinfo}' ";
$row = sql_fetch($sql);
if ($row['memeber_id']) {
    alert_close("입력하신 본인확인 정보로 가입된 내역이 존재합니다.\\n회원아이디 : ".$row['memeber_id']);
}
